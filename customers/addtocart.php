...
<?php
  ...
  if(isset($_GET['productid']) && isset($_GET['emailid']) ){
        $productid = $_GET['productid'];
         $emailid = $_GET['emailid'];
    
    $items = "SELECT * FROM `products` WHERE `productid` = '$productid' " ;
    $results = mysqli_query($dbc, $items);
...
$sellerid = $rows['emailid'];
  ...
  $state = $rows['state'];
  $city = $rows['city'];
  $subtotal = $rows['amount'];
  $delivery = $rows['delivery'];
  
 
 
 
  $insert = mysqli_query($dbc, "INSERT INTO `cart` VALUES (NULL, '$email', '$sellerid' , '$productid', '$productname', '$sellername', '$file', '$brandname', '$category', '$subcategory', '$amount', '$subtotal' , '$producttype', '$productsize', '$color', '$cartquantity', '$delivery' , '$phoneno', '$state', '$city', '$description', now() ) ");
...
<script>
     ...
     swal({
           text: "Success",
   title: "<?php echo $productname;  ?> Added to Cart ",
...
<?php
 ...
 $update = mysqli_query($dbc, " UPDATE `products` SET `quantity` = '$quantity'-1 WHERE `productid` = '$productid'");
