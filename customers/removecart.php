...
<?php  
    if(isset($_GET['productid'])){
        ...
        $emailid = $_SESSION['login'];
            ...
            $cartid = $row['cartid'];    //get the cartid of the product from the cart table
            
            //select and get the quantity of the product from the products table, so product quantity can be increased when user deletes from cart
            $query = mysqli_query($dbc, "SELECT * FROM products WHERE productid='$productid'");
            $row = mysqli_fetch_array($query);
            $oldquantity = $row['quantity'];
            $productname = $row['productname'];
            //check if item was added to cart by the user
            if($numrows > 0){
                $delete = mysqli_query($dbc, "DELETE FROM cart WHERE productid='$productid' AND emailid='$emailid' AND cartid='$cartid'");
                
                //update product quantity in the products table
                $newquantity = $oldquantity+$cartquantity;  //this is the new qauntity that will be stored for the product in the products table
...
<script>
     ...
     swal({
           text: "Success",
   title: "<?php echo $productname;  ?> has been Removed ",
...
<?php
                ...
                //redirect to the logout page
                 echo "<script>window.location.replace('opt.php?action=logout');</script>";
            }
        }
