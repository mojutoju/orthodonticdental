...
<?php
  ...
  $or = mysqli_query($dbc,"SELECT * FROM `categorymaster` WHERE 1   ");
...
<h6 style="font-weight:800;"> <?php echo $categoryname; ?> </h6>
<hr>
...
$osr = mysqli_query($dbc,"SELECT * FROM `subcategory` WHERE 1  ");
if(mysqli_num_rows($osr) > 0){
    $ords = 1;
    while($rows = mysqli_fetch_assoc($osr)){
$categorynames = $rows['categoryname'];
...
$totalsubcategory = "SELECT count(*) FROM `products` WHERE `category` = '$categoryname' AND `subcategory` = '$subcategoryname'  ";
$subcategories = mysqli_query($dbc, $totalsubcategory);
$subcategorytotal = mysqli_fetch_array($subcategories);
     

if($categoryname == $categorynames ){
?>


<a href="subcategory.php?subid=<?php echo $cid; ?>" style="color: black;"> <?php echo $subcategoryname; ?> (<?php echo reset($subcategorytotal); ?>) </a> 
<hr>
<?php
}else{}
?>
